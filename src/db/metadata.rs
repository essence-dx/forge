...
pub const MIRROR_RUNS_TABLE: redb::TableDefinition<&str, &[u8]> =
    redb::TableDefinition::new("mirror_runs");
pub const JOBS_TABLE: redb::TableDefinition<&str, &[u8]> = redb::TableDefinition::new("jobs");
...
impl MetadataDb {
    pub fn create(path: &Path) -> Result<Self> {
        ...
        let write_txn = db.begin_write().context("begin write transaction")?;
        {
            write_txn
                ...
                .context("open FILES_TABLE")?;
            write_txn
                .open_table(CHUNKS_TABLE)
                .context("open CHUNKS_TABLE")?;
            write_txn
            ...
            write_txn
                .open_table(MIRROR_RUNS_TABLE)
                .context("open MIRROR_RUNS_TABLE")?;
            write_txn
                .open_table(JOBS_TABLE)
                .context("open JOBS_TABLE")?;
        }
    ...
    pub fn open(path: &Path) -> Result<Self> {
        let db =
            redb::Database::open(path).with_context(|| format!("open redb {}", path.display()))?;
        // Ensure MIRRORS_TABLE exists for older repos that were init'd before it was added.
        {
            let write_txn = db
                .begin_write()
                .context("begin write txn for schema migration")?;
            write_txn
                .open_table(MIRRORS_TABLE)
                .context("ensure MIRRORS_TABLE")?;
            write_txn
                .open_table(MIRROR_RUNS_TABLE)
                .context("ensure MIRROR_RUNS_TABLE")?;
            write_txn
                .open_table(JOBS_TABLE)
                .context("ensure JOBS_TABLE")?;
    ...
    pub fn stage_file(&self, path: &str, entry_bytes: &[u8]) -> Result<()> {
        let write_txn = self.db.begin_write().context("begin write transaction")?;
        {
            ...
            table
                .insert(path, entry_bytes)
                .context("insert staged file")?;
        }
    ...
    pub fn replace_tracked_files(&self, files: &[(String, Vec<u8>)]) -> Result<()> {
        let write_txn = self.db.begin_write().context("begin write transaction")?;
        {
            let mut table = write_txn
                .open_table(FILES_TABLE)
            ...
            for entry in table.iter().context("iterate files table")? {
                let (key, _) = entry.context("read files row")?;
                existing_keys.push(key.value().to_string());
            }
            for key in existing_keys {
                table.remove(key.as_str()).context("remove tracked file")?;
            }
            for (path, entry_bytes) in files {
                table
                    .insert(path.as_str(), entry_bytes.as_slice())
                    .context("insert tracked file")?;
            }
        }
        write_txn.commit().context("commit replace tracked files")?;
        Ok(())
    }
    ...
    pub fn store_mirror_targets(&self, file_path: &str, targets_json: &[u8]) -> Result<()> {
        let write_txn = self.db.begin_write().context("begin write transaction")?;
        {
            let mut table = write_txn
                .open_table(MIRRORS_TABLE)
                .context("open mirrors table")?;
            table
                .insert(file_path, targets_json)
                .context("insert mirror targets")?;
        }
    ...
    pub fn replace_mirror_targets(&self, targets: &[(String, Vec<u8>)]) -> Result<()> {
        let write_txn = self.db.begin_write().context("begin write transaction")?;
        {
            let mut table = write_txn
                .open_table(MIRRORS_TABLE)
            ...
            let mut existing_keys = Vec::new();
            for entry in table.iter().context("iterate mirrors table")? {
                let (key, _) = entry.context("read mirrors row")?;
                existing_keys.push(key.value().to_string());
            }
            for key in existing_keys {
                table.remove(key.as_str()).context("remove mirror target")?;
            }
            for (file_path, json_bytes) in targets {
                table
                    .insert(file_path.as_str(), json_bytes.as_slice())
                    .context("insert mirror target")?;
            }
        }
        write_txn
            .commit()
            .context("commit replace mirror targets")?;
        Ok(())
    }

    pub fn store_mirror_run(&self, commit_id: &str, run_json: &[u8]) -> Result<()> {
        let write_txn = self.db.begin_write().context("begin write transaction")?;
        {
            let mut table = write_txn
                .open_table(MIRROR_RUNS_TABLE)
                .context("open mirror runs table")?;
            table
                .insert(commit_id, run_json)
                .context("insert mirror run")?;
        }
        write_txn.commit().context("commit mirror run")?;
        Ok(())
    }

    pub fn get_mirror_run(&self, commit_id: &str) -> Result<Option<Vec<u8>>> {
        let read_txn = self.db.begin_read().context("begin read transaction")?;
        let table = read_txn
            .open_table(MIRROR_RUNS_TABLE)
            .context("open mirror runs table")?;
        Ok(table
            .get(commit_id)
            .context("read mirror run")?
            .map(|v| v.value().to_vec()))
    }

    pub fn list_mirror_runs(&self) -> Result<Vec<(String, Vec<u8>)>> {
        let read_txn = self.db.begin_read().context("begin read transaction")?;
        let table = read_txn
            .open_table(MIRROR_RUNS_TABLE)
            .context("open mirror runs table")?;
        let mut out = Vec::new();
        for entry in table.iter().context("iterate mirror runs table")? {
            let (key, val) = entry.context("read mirror run row")?;
            out.push((key.value().to_string(), val.value().to_vec()));
        }
        Ok(out)
    }
    ...
    pub fn store_job(&self, job_id: &str, job_json: &[u8]) -> Result<()> {
        let write_txn = self.db.begin_write().context("begin write transaction")?;
        {
            let mut table = write_txn
                .open_table(JOBS_TABLE)
                .context("open jobs table")?;
            table.insert(job_id, job_json).context("insert job")?;
        }
        write_txn.commit().context("commit job")?;
        Ok(())
    }

    pub fn get_job(&self, job_id: &str) -> Result<Option<Vec<u8>>> {
        let read_txn = self.db.begin_read().context("begin read transaction")?;
        let table = read_txn.open_table(JOBS_TABLE).context("open jobs table")?;
        Ok(table
            .get(job_id)
            .context("read job")?
    ...
    pub fn list_jobs(&self) -> Result<Vec<(String, Vec<u8>)>> {
        let read_txn = self.db.begin_read().context("begin read transaction")?;
        let table = read_txn.open_table(JOBS_TABLE).context("open jobs table")?;
        let mut out = Vec::new();
        for entry in table.iter().context("iterate jobs table")? {
            let (key, value) = entry.context("read jobs row")?;
