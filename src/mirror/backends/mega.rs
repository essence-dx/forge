...
impl MirrorBackend for MegaBackend {
    ...
    async fn upload(
        ...
        let sid = login_json[0]["tsid"]
            ...
            .ok_or_else(|| {
                MirrorError::Upload(format!(
                    "mega login failed: {}",
                    serde_json::to_string_pretty(&login_json).unwrap_or_default()
                ))
            })?
            .to_string();
        ...
        let up_json: serde_json::Value = up_resp
            .json()
            .await
        ...
        let upload_url = up_json[0]["p"]
            ...
            .ok_or_else(|| {
                MirrorError::Upload(format!(
                    "mega upload url missing: {}",
                    serde_json::to_string_pretty(&up_json).unwrap_or_default()
                ))
            })?
            .to_string();
        ...
        let resp = client.post(&full_url).body(data).send().await?;
