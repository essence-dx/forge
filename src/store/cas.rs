...
impl ChunkStore {
    ...
    pub fn chunk_path(&self, hash: &blake3::Hash) -> PathBuf {
        ...
        self.base_dir.join(&hex_hash[0..2]).join(&hex_hash[2..])
    }
    ...
    pub fn store(&self, hash: &blake3::Hash, compressed_data: &[u8]) -> Result<bool> {
        ...
        fs::write(&tmp, compressed_data)
            .with_context(|| format!("write temp chunk {}", tmp.display()))?;
    ...
    pub fn total_size(&self) -> Result<u64> {
            ...
            .filter(|e| e.file_type().is_file())
        {
            total += entry
                .metadata()
                .with_context(|| "metadata read failed")?
                .len();
        }
        Ok(total)
    }
