...
fn print_diff(
    old_map: &BTreeMap<String, FileEntry>,
    new_map: &BTreeMap<String, FileEntry>,
        ...
        match (old_map.get(&path), new_map.get(&path)) {
            ...
            (Some(old_entry), Some(new_entry)) => {
                if old_entry.file_hash != new_entry.file_hash {
                    let old_set: BTreeSet<[u8; 32]> =
                        old_entry.chunks.iter().map(|c| c.hash).collect();
                    let new_set: BTreeSet<[u8; 32]> =
                        new_entry.chunks.iter().map(|c| c.hash).collect();
                    ...
                    println!(
                        ...
                        path, old_entry.size, new_entry.size, changed, reuse_pct
                    );
                }
            }
            (None, None) => {}
